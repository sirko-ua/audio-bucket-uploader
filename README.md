...
З `--standalone` (типово) завантажувач також шукає окремі файли поруч із відео-джерелом:

- **Аудіо:** `wav`, `mp3`, `aac`, `flac`, `ogg`, `m4a`, `opus`, `ac3`, `eac3`, `ac4`, `dts`, `dtshd`, `truehd`, `mlp`, `thd`
- **Субтитри:** `ass`, `srt`, `pgs`, `sup`

Такий файл завантажується, лише якщо його мову можна визначити з назви (наприклад, `Movie.uk.srt` або `Movie_track2_[ukr]_DELAY 0ms.eac3`) чи MediaInfo, а відеофайл-сусід із такою самою основою має `unique_id` у MediaInfo. Відео-джерело не мусить містити відповідну доріжку: MediaInfo самостійного файлу додається як додаткова доріжка, але `unique_id` справжнього відео зберігається. Самостійні файли-джерела ніколи не видаляються.
