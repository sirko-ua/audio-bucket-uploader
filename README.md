# Audio Bucket Uploader

Витягує аудіодоріжки й доріжки субтитрів із файлів `.mkv` за мовою та завантажує їх до Audio Bucket.

## Огляд

- знаходить один файл `.mkv` або рекурсивно сканує каталог на наявність файлів `.mkv`
- витягує відповідні аудіодоріжки й доріжки субтитрів за допомогою `mkvextract`
- називає витягнуті файли за шаблоном:

```text
{original_movie_name}_track{track_id_inside_container}.{detected_extension}
```

- завантажує кожну витягнуту доріжку до Audio Bucket як чернетку або публічну доріжку через `POST /api/uploader`
- перед завантаженням обчислює BLAKE3-256 хеш і перевіряє його через `POST /api/uploader/hash-check`, щоб не створювати дублікати
- передає у запиті на завантаження витягнутий `media_file`, оригінальні JSON/текст MediaInfo для MKV, `ID` MediaInfo як `track_id_inside_container` та вибране значення `visibility`
- показує поступ завантаження для кожної витягнутої доріжки під час її надсилання (вивід витягання перехоплюється, щоб про помилку утиліти можна було повідомити з її причиною)
- з параметром `--verbose` виводить докладні відомості про виявлення, HTTP-запити, результати завантаження й очищення
- видаляє кожен витягнутий файл, щойно з ним завершено роботу — завантажено, визнано дублікатом чи не вдалося, — якщо не вказано `--keep-extracted`, щоб тривалий запуск не заповнив тимчасовий каталог
- за потреби також завантажує окремі (самостійні) аудіофайли й файли субтитрів, що лежать поряд зі своїм відеофайлом-джерелом (див. [Самостійні файли](#самостійні-файли))
- запам'ятовує вже виконану роботу, тож після перезапуску продовжує з місця зупинки, а не витягає та не перевіряє хеші повторно (див. [Історія та відновлення](#історія-та-відновлення))
- переживає пошкоджені файли, невдалі витягання та мережеві помилки: записує їх і працює далі (див. [Помилки](#помилки))

## Аргументи

| Аргумент | Обов’язковий | Типове значення | Опис |
| --- | --- | --- | --- |
| `--api-key` | Так | немає | API-ключ користувача Audio Bucket. Він надсилається як bearer-токен у запиті на завантаження. |
| `--api-url` | Так | немає | URL кінцевої точки завантажувача Audio Bucket, наприклад `https://audio-bucket.site/api/uploader`. |
| `--input` | Ні | `/input` | Шлях до одного файлу `.mkv` або каталогу з файлами `.mkv`. Каталоги скануються рекурсивно. |
| `--audio-language` | Ні | `uk` | Мова цільової аудіодоріжки. Передавайте параметр кілька разів або використовуйте значення, розділені комами, наприклад `--audio-language uk --audio-language en` чи `--audio-language uk,en`. |
| `--subtitle-language` | Ні | `all` | Мова цільової доріжки субтитрів. Передавайте параметр кілька разів або використовуйте значення, розділені комами. `all` завантажує кожну доріжку субтитрів незалежно від мови. |
| `--output-dir` | Ні | Тимчасовий каталог ОС | Каталог, до якого витягнуті доріжки записуються перед завантаженням. У macOS і Linux це зазвичай `/tmp`; у Windows використовується стандартне розташування тимчасових файлів зі змінних середовища ОС. |
| `--keep-extracted` | Ні | `false` | Зберігати витягнуті файли замість того, щоб видаляти їх, щойно з ними завершено роботу (завантажено, визнано дублікатом чи не вдалося). |
| `--visibility` | Ні | `public` | Видимість завантажених доріжок: `draft` або `public`. |
| `--standalone`, `--no-standalone` | Ні | `true` | Також знаходити й завантажувати окремі (самостійні) аудіофайли та файли субтитрів. Див. [Самостійні файли](#самостійні-файли). Скористайтеся `--no-standalone`, щоб обробляти лише файли `.mkv`. |
| `--state-dir` | Ні | `~/.audio-bucket-uploader` | Каталог з історією запусків (`history.sqlite3`) і журналом помилок (`failures.jsonl`). Можна також задати змінною середовища `UPLOADER_STATE_DIR`; у Docker-образі типове значення — `/state`. |
| `--retry-failed` | Ні | `false` | Забути попередні помилки перед запуском, щоб файли, від яких відмовилися після 3 невдалих спроб, спробувати ще раз. Використовуйте після усунення причини. |
| `--verbose`, `--no-verbose` | Ні | `false` | Виводити докладну інформацію про виявлення файлів, HTTP-запити, результати завантаження та очищення. Виявлені файли показуються в одноколонкових таблицях повних шляхів. Для запитів виводяться лише метод, шлях і код відповіді — без тіла запиту. Для невдалого запиту також виводиться короткий фрагмент повідомлення про помилку від сервера, щоб можна було з'ясувати причину відмови. |

Мовні фільтри нормалізуються, а для мов на кшталт `uk`, `ukr` та `ukrainian` підтримуються поширені псевдоніми.

## Історія та відновлення

Витягання доріжки та обчислення її хешу коштують хвилин, а повторне запитання до сервера про хеш, на який він уже відповів, — це зайвий запит до нього. Тому завантажувач записує кожен оброблений файл-джерело та кожну завершену доріжку до `history.sqlite3` у каталозі стану, і після перезапуску пропускає їх **до** запуску MediaInfo, до витягання й до звернення до сервера.

- Кожен запис прив'язаний до шляху файлу разом з його розміром і часом зміни, тож заміна або переконвертація файлу знову його обробляє.
- Доріжки, які вже завантажено або які перевірка хешу визнала опублікованими, більше ніколи не перевіряються повторно.
- Файли без відповідних доріжок і самостійні файли, пропущені з причини, що не може змінитися сама собою (немає мови в імені, мова не запитана), також запам'ятовуються. Зміна `--audio-language` чи `--subtitle-language` змушує переглянути їх знову; самостійний файл, пропущений через відсутність відео-джерела, переглядається щоразу, бо відео могли додати пізніше.
- Помилки повторюються під час наступних запусків до 3 разів. Після цього від файлу **відмовляються**: у кожному наступному запуску його показано як `ERROR`, враховано в підсумку, і код виходу стає ненульовим — його ніколи не вважають тихо завершеним. Коли причину усунуто, `--retry-failed` очищає ці помилки й пробує ще раз.
- Щоб обробити бібліотеку з нуля, видаліть каталог стану (або вкажіть новий шлях у `--state-dir`).
- Видимість визначається першим завантаженням. Повторний запуск з іншим `--visibility` не змінить уже опубліковану доріжку: перевірка хешу на сервері повідомить, що вона існує, незалежно від видимості.

**У Docker обов'язково змонтуйте каталог стану**, інакше історія зникне разом з контейнером:

```bash
docker run --rm -v /path/to/movies:/input:ro -v /path/to/state:/state ...
```

Допоміжні скрипти `ukrab-uploader.sh` і `ukrab-uploader.bat` роблять це за вас, зберігаючи стан у `.audio-bucket-uploader` всередині каталогу з медіа. Кожній бібліотеці — **власний каталог стану**: у контейнері кожна бібліотека монтується як `/input`, тож спільний каталог стану дозволив би сплутати дзеркальну копію бібліотеки з уже завантаженою.

## Помилки

Один пошкоджений `.mkv`, невдале витягання, відхилене завантаження чи мережевий збій ніколи не завершують роботу. Кожну таку подію записано в журнал, а завантажувач переходить до наступного файлу. Зупиняють його лише дві речі: **більше нічого завантажувати** або **сервер недоступний**.

- Кожну помилку додано до `failures.jsonl` у каталозі стану — один JSON-об'єкт на рядок, з часом, шляхом, етапом і текстом помилки (включно з виводом утиліти, що впала), тож пошкоджений файл можна дослідити згодом.
- Пошкоджена доріжка більше не тягне за собою справні: якщо витягання всіх доріжок файлу за один прохід не вдалося, кожну доріжку витягають окремо.
- Файл, який `mkvmerge` не може прочитати, позначено як нечитабельний, а не тихо зараховано до «немає відповідних доріжок».
- Запити повторюються до 5 разів з експоненційною затримкою. Перш ніж надіслати завантаження ще раз — після тайм-ауту, розірваного з'єднання чи `5xx` — завантажувач запитує перевірку хешу, чи файл усе-таки дійшов, тож загублена відповідь ніколи не публікує ту саму доріжку двічі.
- Відмова щодо одного файлу (`400`, `403`, `404`, `422` — наприклад «немає релізу для цього unique_id») — це відповідь про цей файл, а не вирок усьому запуску: її записано, і робота триває, скільки б файлів поспіль не відхилили.
- Робота припиняється з кодом виходу `3` лише тоді, коли проблема в самому сервері: він відхиляє API-ключ (`401`), **найперший** запит повертає `403`/`404` (хибний `--api-url` чи ключ, коли ще ніщо не спрацювало), одна кінцева точка кілька разів поспіль зазнає невдачі після всіх повторів, або сервер через `Retry-After` просить зачекати довше, ніж дозволяє повтор. Одне розірване з'єднання спершу вважається невдачею саме цього файлу: на справному сервері зазвичай так і є (проксі відхиляє завеликий запит), а зупинка через нього змушувала б надсилати цей файл заново при кожному перезапуску.
- Скільки саме невдач поспіль потрібно — залежить від того, що повернулося. З'єднання, яке не відповідає, означає, що сервера немає: **3** поспіль зупиняють роботу. `5xx` — інша річ: сервер відповів, отже він працює й доступний — він не впорався саме з цим запитом. Тека з файлами, які кінцева точка не може перетравити, — це низка відмов щодо окремих файлів, а не збій сервера, тож потрібно **12** поспіль. Обидва лічильники скидаються після першої вдалої відповіді від цієї кінцевої точки.
- Невдалу спробу прочитати файл повторюють 3 рази, перш ніж звинуватити сам файл. Диск, що заснув, перепідключення NAS чи файл, який завантажувач ще дописує, змушують справний контейнер не пройти одне читання `mediainfo`/`mkvmerge` і пройти наступне — без повтору такий збій з'їв би одну з 3 спроб файлу й зрештою призвів би до відмови від нього назавжди.
- Відео зі змінною частотою кадрів (VFR) не зберігає частоту в заголовках, і сервер відхиляє завантаження без неї («Could not parse original video FPS from MediaInfo»). Такі файли автоматично перечитуються повністю, щоб обчислити справжню частоту й надіслати її. Повне читання проходить весь файл, тож запускається лише для тих файлів, яким це потрібно, і саме по собі ніколи не провалює файл: якщо частоту так і не вдалося обчислити, надсилається початковий MediaInfo, а останнє слово лишається за сервером.
- Коди виходу: `0` — завершено, `1` — завершено з помилками або з файлами, від яких відмовилися (див. `failures.jsonl`), `2` — помилка в аргументах командного рядка (argparse), `3` — зупинено, бо сервер недоступний, `130` — перервано, `143` — завершено сигналом (`docker stop`, після очищення).

## Самостійні файли

З `--standalone` (типове значення) завантажувач також обробляє окремі аудіофайли й файли субтитрів, а не лише доріжки всередині контейнерів `.mkv`:

- **Аудіо:** `wav`, `mp3`, `aac`, `flac`, `ogg`, `m4a`, `opus`, `ac3`, `eac3`, `ac4`, `dts`, `dtshd`, `truehd`, `mlp`, `thd`
- **Субтитри:** `ass`, `srt`, `pgs`, `sup`

Кінцева точка завантажувача ідентифікує доріжку за `unique_id` з MediaInfo відео-джерела й зчитує тип і мову доріжки з MediaInfo того самого відео. Тому самостійний файл завантажується лише за виконання обох умов нижче, інакше він пропускається з поясненням:

1. Його мову можна визначити з імені файлу (наприклад, `Movie.uk.srt`, `Show.en-GB.forced.srt`, `Movie_track2_[ukr]_DELAY 0ms.eac3`) або з MediaInfo.
2. У тому самому каталозі є **відеофайл-сусід** з такою самою основою імені (наприклад, `Movie.mkv` поряд із `Movie.uk.srt` чи `Movie_track2.eac3`). Відео повинно мати `unique_id` у MediaInfo — `.mkv` його має, більшість `.mp4` — ні.

Відео-джерело **не обов'язково** має вже містити відповідну доріжку. Зовнішні субтитри зазвичай не мають відповідника всередині контейнера (`Movie.mkv` має українську аудіодоріжку, але не має українських субтитрів), тому MediaInfo самого самостійного файлу додається до MediaInfo відео-джерела як додаткова доріжка, а `track_id_inside_container` вказує на неї. Загальний `unique_id` залишається таким, як у справжнього відео-джерела, тож завантаження потрапляє до правильного релізу, а кінцева точка зчитує правильні тип і мову.

Самостійні файли-джерела ніколи не видаляються (`--keep-extracted` на них не поширюється).

## Перевірка дублікатів

Перед кожним завантаженням програма обчислює 64-символьний BLAKE3-256 дайджест витягнутого файлу та надсилає його до `POST /api/uploader/hash-check`. Якщо API повертає `{"exists": true}`, файл вважається вже опублікованим і завантаження пропускається.

Перевірка залежить лише від хешу файлу й не змінюється через `--visibility`: вона виконується однаково для `public` і `draft`. Параметр `--visibility` використовується лише під час фактичного завантаження, коли API повернув `{"exists": false}`. Тому повторний запуск з іншою видимістю також буде пропущено, якщо API вже знайшов цей хеш.

Перевірка застосовується і до самостійних файлів.

## Найпростіший спосіб запуску (macOS і Linux)

Спочатку встановіть і запустіть [Docker](https://www.docker.com/get-started/). Потім завантажте допоміжний скрипт і дозвольте його виконання:

```bash
curl -fsSLO https://raw.githubusercontent.com/sirko-ua/audio-bucket-uploader/main/scripts/ukrab-uploader.sh
chmod +x ukrab-uploader.sh
```

Або скористайтеся `wget`:

```bash
wget https://raw.githubusercontent.com/sirko-ua/audio-bucket-uploader/main/scripts/ukrab-uploader.sh
chmod +x ukrab-uploader.sh
```

Запустіть його з вашим API-ключем і шляхом до одного файлу `.mkv` або каталогу з файлами `.mkv`. Каталоги скануються рекурсивно:

```bash
./ukrab-uploader.sh <your_api_key> /path/to/movie-or-directory
```

Типово завантаження є публічними. Щоб натомість створювати чернетки, додайте `draft`:

```bash
./ukrab-uploader.sh <your_api_key> /path/to/movie-or-directory draft
```

Також працює форма з іменованими параметрами: `./ukrab-uploader.sh --api-key <your_api_key> --input /path/to/movie-or-directory --visibility draft`. Додайте `--verbose` до будь-якої форми для докладного виводу (або `--no-verbose`, щоб явно вибрати стислий вивід). Допоміжний скрипт автоматично завантажує образ `ghcr.io/sirko-ua/audio-bucket-uploader:latest`, коли це потрібно, і використовує `https://ukrab.work/api/uploader`. Він зберігає стандартні значення завантажувача: українська аудіодоріжка, усі субтитри, тимчасові витягнуті файли та стислий вивід.

## Найпростіший спосіб запуску (Windows)

Спочатку встановіть і запустіть [Docker Desktop](https://www.docker.com/products/docker-desktop/). У PowerShell завантажте допоміжний скрипт для Windows:

```powershell
Invoke-WebRequest https://raw.githubusercontent.com/sirko-ua/audio-bucket-uploader/main/scripts/ukrab-uploader.bat -OutFile ukrab-uploader.bat
```

Запустіть його з вашим API-ключем і шляхом до одного файлу `.mkv` або каталогу з файлами `.mkv`:

```powershell
.\ukrab-uploader.bat <your_api_key> "C:\path\to\movie-or-directory"
```

Типово завантаження є публічними. Додайте `draft` як останній аргумент, щоб натомість створювати чернетки:

```powershell
.\ukrab-uploader.bat <your_api_key> "C:\path\to\movie-or-directory" draft
```

Також працює форма з іменованими параметрами: `.\ukrab-uploader.bat --api-key <your_api_key> --input "C:\path\to\movie-or-directory" --visibility draft`. Додайте `--verbose` до будь-якої форми для докладного виводу (або `--no-verbose`, щоб явно вибрати стислий вивід).

## Запуск Docker безпосередньо

Завантажте опублікований образ:

```bash
docker pull ghcr.io/sirko-ua/audio-bucket-uploader:latest
```

Мінімально необхідні параметри:

```bash
docker run --rm \
  -v /path/to/movies:/input:ro \
  ghcr.io/sirko-ua/audio-bucket-uploader:latest \
  --api-key <your_api_key> \
  --api-url https://audio-bucket.site/api/uploader
```

Тут використовується типове значення `--input /input`, тому завантажувач сканує змонтований каталог із фільмами.

Повна команда з усіма доступними параметрами:

```bash
docker run --rm \
  -v /path/to/movies:/input:ro \
  -v /path/to/extracted:/output \
  -v /path/to/state:/state \
  ghcr.io/sirko-ua/audio-bucket-uploader:latest \
  --api-key <your_api_key> \
  --api-url https://audio-bucket.site/api/uploader \
  --input /input \
  --audio-language uk \
  --subtitle-language all \
  --output-dir /output \
  --state-dir /state \
  --keep-extracted \
  --visibility public \
  --verbose
```

## Локальний запуск

Потрібні Python 3.10 або новіший, а також `mediainfo` і `mkvtoolnix` (`mkvmerge`, `mkvextract`) у `PATH`. Спочатку встановіть залежності Python:

```bash
python -m pip install -r requirements.txt
```

Мінімально необхідні параметри:

```bash
python -m uploader \
  --api-key <your_api_key> \
  --api-url https://audio-bucket.site/api/uploader
```

Повна команда:

```bash
python -m uploader \
  --api-key <your_api_key> \
  --api-url https://audio-bucket.site/api/uploader \
  --input /media/movies \
  --audio-language uk,en \
  --subtitle-language all \
  --output-dir ./extracted \
  --keep-extracted \
  --visibility public \
  --verbose
```
